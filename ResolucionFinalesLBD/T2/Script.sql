...
CREATE TABLE Asignaturas(
    ...
    tipo      VARCHAR(20)    NOT NULL
              CHECK (tipo IN ('Obligatoria', 'Opcional')),
    PRIMARY KEY (codigo)
)ENGINE=INNODB
;
...
CREATE TABLE Detalle(
    ...
    INDEX Ref44(idCurricula),
    INDEX Ref65(codigo), 
    CONSTRAINT RefCurricula4 FOREIGN KEY (idCurricula)
    REFERENCES Curricula(idCurricula),
    CONSTRAINT RefAsignaturas5 FOREIGN KEY (codigo)
...
CREATE TABLE Propuestas(
    ...
    idEtapa        INT    NOT NULL,
    idCurricula    INT    NOT NULL,
    PRIMARY KEY (idEscuela, idEtapa, idCurricula), 
    INDEX Ref11(idEscuela),
    INDEX Ref32(idEtapa),
    INDEX Ref43(idCurricula), 
    CONSTRAINT RefEscuelas1 FOREIGN KEY (idEscuela)
    REFERENCES Escuelas(idEscuela),
    CONSTRAINT RefEtapas2 FOREIGN KEY (idEtapa)
    REFERENCES Etapas(idEtapa),
    CONSTRAINT RefCurricula3 FOREIGN KEY (idCurricula)
    REFERENCES Curricula(idCurricula)
)ENGINE=INNODB
;

-- Creamos trigger en escuelas.
DROP TRIGGER IF EXISTS `DNI`.`Escuelas_AFTER_INSERT`;

DELIMITER $$
USE `DNI`$$
CREATE TRIGGER `DNI`.`Escuelas_AFTER_INSERT` AFTER INSERT ON `Escuelas` FOR EACH ROW
BEGIN
INSERT INTO Auditoria VALUES(0, NOW(), SUBSTRING_INDEX(USER(),'@',1), SUBSTRING_INDEX(USER(),'@',-1), 
    'I',NEW.idEscuela, NEW.codigo, NEW.nombre, NEW.domicilio, NEW.tipo);
END$$
DELIMITER ;
...
USE `DNI`;
DROP procedure IF EXISTS `DNI`.`NuevaEscuela`;
;

DELIMITER $$
USE `DNI`$$
CREATE  PROCEDURE `NuevaEscuela`(pidEscuela INTEGER,pCodigo VARCHAR(10), pNombre VARCHAR(100), pDomicilio VARCHAR(70), pTipo VARCHAR(10), OUT pMensaje VARCHAR(255))
SALIR:BEGIN
/*
	Permite insertar una Escuela. Controla que los parametros sean correctos.
    Controla que el id, codigo y tipo sean los correctos.
	Devuelve Mensaje de exito + ID de la escuela creada o el mensaje de error en pMensaje.
*/
    ...
    -- Controlo pidEscuela
    IF pidEscuela IS NULL OR pidEscuela = 0 THEN
		SELECT 'El ID de la escuela es obligatorio.' INTO pMensaje;
        LEAVE SALIR;
    END IF;
    
    -- Controlo pCodigo
    IF pCodigo IS NULL OR pCodigo = '' THEN
		SELECT 'El codigo es obligatorio.' INTO pMensaje;
        LEAVE SALIR;
    END IF;
	
	-- Controlo pNombre
    IF pNombre IS NULL OR pNombre = '' THEN
		SELECT 'El nombre es obligatorio.' INTO pMensaje;
        LEAVE SALIR;
    END IF;

	-- Controlo pDomicilio
    IF pDomicilio IS NULL OR pDomicilio = '' THEN
		SELECT 'El domicilio es obligatorio.' INTO pMensaje;
        LEAVE SALIR;
    END IF;
    
	-- Controlo pTipo
    IF pTipo IS NULL OR pTipo = '' THEN
		SELECT 'El tipo es obligatorio.' INTO pMensaje;
        LEAVE SALIR;
    END IF;
    
	-- Controlo tipo publica o privada
    IF pTipo != 'Pública' AND pTipo != 'Privada' THEN
		SELECT 'El tipo solo puede ser Pública o Privada.' INTO pMensaje;
        LEAVE SALIR;
    END IF;
	
    -- Controlo idEscuela no repetido
     IF EXISTS(SELECT idEscuela FROM Escuelas WHERE (idEscuela = pidEscuela))
		THEN SELECT 'El ID de escuela ya existe para otra escuela.' INTO pMensaje;
        LEAVE SALIR;
	END IF;
    
	-- Controlo que no exista codigo
    IF EXISTS(SELECT codigo FROM Escuelas WHERE (codigo = pCodigo))
		THEN SELECT 'El codigo de escuela ya existe para otra escuela.' INTO pMensaje;
        LEAVE SALIR;
	END IF;

START TRANSACTION;
	INSERT INTO Escuelas VALUES(pidEscuela,pCodigo, pNombre, pDomicilio, pTipo);
	SELECT CONCAT('Se creo la escuela con el ID: ', pidEscuela) INTO pMensaje;
COMMIT;
END$$
...
CALL NuevaEscuela(null,null,null,null,null, @pMensaje); -- ID de escuela obligatorio
SELECT @pMensaje;

CALL NuevaEscuela(99999,null,null,null,null, @pMensaje); -- codigo obligatorio
SELECT @pMensaje;


CALL NuevaEscuela(99999,2394,null,null,null, @pMensaje); -- nombre obligatorio.
SELECT @pMensaje;


CALL NuevaEscuela(99999,2394,'gomez',null,null, @pMensaje); -- Domicilio obligatorio
SELECT @pMensaje;


CALL NuevaEscuela(99999,2394,'gomez','junin 342',null, @pMensaje); -- Tipo es obligatorio
SELECT @pMensaje;


CALL NuevaEscuela(99999,2394,'gomez','junin 342','Regular', @pMensaje); -- Tipo debe ser publica o privada
SELECT @pMensaje;


CALL NuevaEscuela(54,2394,'gomez','junin 342','Privada', @pMensaje); -- ID de escuela ya existe.
SELECT @pMensaje;


CALL NuevaEscuela(9999,2394,'gomez','junin 342','Privada', @pMensaje); -- OK.
SELECT @pMensaje;
...
DELIMITER $$
CREATE PROCEDURE `sp_RankingCargaCurricula` ()
BEGIN
/*
	Muestra un ranking con las currículas con mayor carga horaria. Muestra el
	nombre de la currícula y la carga horaria total de la misma.
*/

SELECT ROW_NUMBER() OVER (ORDER BY SUM(de.Carga) DESC) AS Ranking, 
cu.nombre , SUM(de.carga) AS Carga FROM Curricula cu INNER JOIN Detalle de 
ON cu.idCurricula = de.idCurricula
GROUP BY cu.nombre
ORDER BY Carga DESC;
...
-- Probamos SP Ranking
CALL sp_RankingCargaCurricula();

-- Creamos la vista CurriculasEscuelas
DROP VIEW IF EXISTS CurriculasEscuelas;
CREATE VIEW CurriculasEscuelas AS SELECT es.codigo AS 'Codigo escuela', es.nombre AS Escuela, et.nombre AS Etapa, 
cu.nombre AS Curricula, a.codigo AS 'Codigo curricula', a.nombre AS Asignatura, a.tipo, de.carga
FROM Propuestas pr 
INNER JOIN Escuelas es ON pr.idEscuela = es.idEscuela
INNER JOIN Etapas et ON pr.idEtapa = et.idEtapa
INNER JOIN Curricula cu ON pr.idCurricula = cu.idCurricula
INNER JOIN Detalle de ON cu.idCurricula = de.idCurricula
INNER JOIN Asignaturas a ON de.codigo = a.codigo
ORDER BY Escuela, Etapa, Curricula;
