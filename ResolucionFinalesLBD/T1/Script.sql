...
-- Author :       jf
--
-- Date Created : Friday, June 18, 2021 11:24:24
...
CREATE TABLE Trabajadores(
    ...
    IDAgencia    INT            NOT NULL,
    PRIMARY KEY (Legajo), 
    UNIQUE INDEX UI_Documento(Documento),
    INDEX Ref85(IDAgencia), 
    CONSTRAINT RefAgencias53 FOREIGN KEY (IDAgencia)
...
CREATE TABLE Pedidos(
    ...
    INDEX Ref31(Remitente),
    INDEX Ref32(Destinatario),
    INDEX Ref63(Legajo), 
    CONSTRAINT RefUsuarios13 FOREIGN KEY (Remitente)
    REFERENCES Usuarios(IDUsuario),
    CONSTRAINT RefUsuarios23 FOREIGN KEY (Destinatario)
    REFERENCES Usuarios(IDUsuario),
    CONSTRAINT RefTrabajadores33 FOREIGN KEY (Legajo)
...
CREATE TABLE Correspondencias(
    ...
    PRIMARY KEY (IDPedido), 
    INDEX Ref57(IDPedido), 
    CONSTRAINT RefPedidos73 FOREIGN KEY (IDPedido)
...
CREATE TABLE Paquetes(
    ...
    PRIMARY KEY (IDPedido), 
    INDEX Ref58(IDPedido), 
    CONSTRAINT RefPedidos83 FOREIGN KEY (IDPedido)
    REFERENCES Pedidos(IDPedido)
)ENGINE=INNODB
;
...
-- 
-- TABLE: PedidosPorAgencias 
--

CREATE TABLE PedidosPorAgencias(
    IDAgencia     INT            NOT NULL,
    IDPedido      VARCHAR(20)    NOT NULL,
    FechaYHora    DATETIME       NOT NULL,
    PRIMARY KEY (IDAgencia, IDPedido), 
    INDEX Ref84(IDAgencia),
    INDEX Ref56(IDPedido), 
    CONSTRAINT RefAgencias43 FOREIGN KEY (IDAgencia)
    REFERENCES Agencias(IDAgencia),
    CONSTRAINT RefPedidos63 FOREIGN KEY (IDPedido)
    REFERENCES Pedidos(IDPedido)
)ENGINE=INNODB
;

-- 
-- TABLE: AuditoriaTrabajadores 
--
...
DROP TRIGGER IF EXISTS `DNI`.`Trabajadores_AFTER_DELETE`;

DELIMITER $$
USE `DNI`$$
CREATE TRIGGER `DNI`.`Trabajadores_AFTER_DELETE` AFTER DELETE ON `Trabajadores` FOR EACH ROW
BEGIN
	INSERT INTO AuditoriaTrabajadores VALUES(0, NOW(), SUBSTRING_INDEX(USER(),'@',1), SUBSTRING_INDEX(USER(),'@',-1), 
    'B',OLD.Legajo, OLD.Documento, OLD.Nombres, OLD.Apellidos, OLD.Ingreso, OLD.Clase, OLD.IDAgencia);
END$$
DELIMITER ;
...
-- Creamos SP para borrar trabajador.
USE `DNI`;
DROP procedure IF EXISTS `BorrarTrabajador`;

DELIMITER $$
USE `DNI`$$
CREATE PROCEDURE `BorrarTrabajador`(pLegajo INTEGER, OUT pMensaje VARCHAR(255))
SALIR:BEGIN
	/*
    Permite borrar un Trabajador. Controla parametros de entrada. Controla que exista
    Controla que no posee pedidos asociados.
    En caso de exito devuelve el ID del trabajador borrado.
	...
	*/
	...
	IF pLegajo IS NULL OR pLegajo = 0 THEN
		SELECT 'El legajo es obligatorio.' INTO pMensaje;
        LEAVE SALIR;
    END IF;

   -- Controlar si existe el Trabajador
    IF NOT EXISTS(SELECT Legajo FROM Trabajadores WHERE Legajo = pLegajo) 
    THEN SELECT 'El trabajador que quiere borrar no existe.' INTO pMensaje;
        LEAVE SALIR; 
	END IF;
    
	-- Controla que el trabajador no este asociado en ningun pedido
    IF EXISTS(SELECT Legajo FROM Pedidos WHERE Legajo = pLegajo) 
		THEN SELECT 'El trabajador posee pedidos asociados.' INTO pMensaje;
        LEAVE SALIR; 
	END IF;
    
    START TRANSACTION;
		DELETE FROM Trabajadores WHERE Legajo = pLegajo;
        SELECT CONCAT('Se ha borrado con exito el trabajador con Legajo: ', pLegajo) INTO pMensaje;
	COMMIT;
END$$
...
CALL BorrarTrabajador(0,@pMensaje); -- El legajo es obligatorio.
SELECT @pMensaje;

CALL BorrarTrabajador(999, @pMensaje); -- El trabajador que quiere borrar no existe.
SELECT @pMensaje;

CALL BorrarTrabajador(320001, @pMensaje); -- El trabajador posee pedidos asociados.
SELECT @pMensaje;

CALL BorrarTrabajador(10001, @pMensaje); -- Se ha borrado con exito el trabajador con Legajo 10001.
SELECT @pMensaje;

-- Controlamos que el trigger funcione
SELECT * FROM AuditoriaTrabajadores;

-- Creamos SP RankingUsuariosPorPedidos

USE `DNI`;
DROP procedure IF EXISTS `DNI`.`RankingUsuariosPorPedidos`;

DELIMITER $$
USE `DNI`$$
CREATE PROCEDURE `RankingUsuariosPorPedidos`(pFechaInicio DATE, pFechaFin DATE)
SALIR: BEGIN
/*
    Permite mostrar un ranking con los usuarios que mas pedidos realizan (por costo)
    entre un rango de fechas. Ordena fechas cruzadas.
*/
DECLARE pAux date;
	
IF pFechaInicio IS NULL THEN
		SELECT 'Debe especificarse la fecha de inicio del rango.' AS Mensaje;
        LEAVE SALIR;
    END IF;
    
	IF pFechaFin IS NULL THEN
		SELECT 'Debe especificarse la fecha de finalizacion del rango.' AS Mensaje;
        LEAVE SALIR;
    END IF;
...
SELECT ROW_NUMBER() OVER (ORDER BY SUM(p.Costo) DESC, u.Apellidos, u.Nombres) AS Ranking, 
u.IDUsuario AS IdUsuario, u.Apellidos AS Apellidos, u.Nombres AS Nombres, SUM(p.Costo) AS Costo FROM Usuarios u
INNER JOIN Pedidos p ON u.IdUsuario = p.Remitente OR u.IdUsuario = p.Destinatario
INNER JOIN tmp tmp ON p.IDPedido = tmp.IDPedido
...
-- Probamos SP RankingUsuariosPorPedidos
CALL RankingUsuariosPorPedidos('2020-05-05', '2030-05-05'); -- Si no contiene datos para el rango de fecha dado, muestra la lista vacia.
CALL RankingUsuariosPorPedidos(NOW(), '2000-05-05'); -- Soluciona la fecha cruzada.
CALL RankingUsuariosPorPedidos('2000-05-05', NOW()); -- Similar a la anterior, pero sin fechas cruzadas.

-- Crear vista llamada Ruta Paquete 

DROP VIEW IF EXISTS RutaPaquete;

CREATE VIEW RutaPaquete AS
SELECT paq.IDPedido, age.Direccion, age.Localidad, ppa.FechaYHora, 
CONCAT(us1.Apellidos,', ', us1.Nombres) AS Remitente, 
CONCAT(us2.Apellidos, ', ',us2.Nombres) AS Destinatario
FROM Paquetes paq INNER JOIN Pedidos ped ON paq.IDPedido = ped.IDPedido
INNER JOIN Usuarios us1 ON ped.Remitente = us1.IDUsuario
INNER JOIN Usuarios us2 ON ped.Destinatario = us2.IDUsuario
INNER JOIN PedidosPorAgencias ppa ON ped.IDPedido = ppa.IDPedido
INNER JOIN Agencias age ON ppa.IDAgencia = age.IDAgencia
ORDER BY ppa.FechaYHora DESC, Remitente, Destinatario;

-- Mostramos lo que contiene la vista Ruta Paquete
